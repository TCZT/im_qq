...
class ServerProcess extends Thread {
	...
	public static Vector onlineUser = new Vector(10, 5);
	private static Vector socketUser = new Vector(10, 5);

	private String strReceive, strKey;
	private StringTokenizer st;

	private final String USERLIST_FILE = "C:\\Users\\dell\\workspace\\im_qq\\user.txt"; // 设定存放用户信息的文件
	...
	private void register() throws IOException {
		...
		if (isExistUser(name)) {
			...
			out.println("regwarning|该用户已存在，请改名!");
		} else {
	...
	private void login() throws IOException {
		...
		for (int i = 0; i < onlineUser.size(); i++) {
			if (onlineUser.elementAt(i).equals(name)) {
				...
				out.println("loginwarning|" + name + "已经登陆聊天室");
			}
		}
		...
		if (!succeed) {
			out.println("loginwarning|" + name + "账号密码错误 登陆失败!");
			log("用户" + name + "登陆失败！" + t.toLocaleString());
			System.out.println("[SYSTEM] " + name + " 账号密码错误 登陆失败!");
		}
	}
	...
	private void talk() throws IOException {
		...
		System.out.println("[TALK]" + strSender + "对" + strReceiver + "说：" + strTalkInfo);
		Socket socketSend;
		PrintWriter outSend;
		Date t = new Date();
		...
		log("用户" + strSender + "对 " + strReceiver + "说:" + strTalkInfo + t.toLocaleString());
